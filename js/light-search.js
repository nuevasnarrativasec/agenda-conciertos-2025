  ...
  input.addEventListener('input', () => {
    ...
    for (const element of elements) {
      if (element.innerHTML.toLowerCase().includes(input.value.toLowerCase())) {
          element.style.display = options.display ? options.display : 'inline-block';
          result++;
    ...
    if(options.displayontype) {
      if(input.value.length >= options.displayontype) {
        options.parent.style.display = options.displayparent ? options.displayparent : 'inline-block';
      } else {
